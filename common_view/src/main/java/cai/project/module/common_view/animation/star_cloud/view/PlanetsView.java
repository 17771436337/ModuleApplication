package cai.project.module.common_view.animation.star_cloud.view;
...
import cai.project.module.common_view.animation.star_cloud.PlanetCalculator;
import cai.project.module.common_view.animation.star_cloud.adapter.NullPlanetAdapter;
import cai.project.module.common_view.animation.star_cloud.adapter.PlanetAdapter;
import cai.project.module.common_view.animation.star_cloud.model.PlanetModel;

/**
 * 星球云
 */
public class PlanetsView extends ViewGroup implements Runnable, PlanetAdapter.OnDataSetChangeListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PlanetsView);
            mode = typedArray.getInteger(R.styleable.PlanetsView_autoScrollMode, MODE_DISABLE);
            setManualScroll(typedArray.getBoolean(R.styleable.PlanetsView_manualScroll, true));
            mAngleX = typedArray.getFloat(R.styleable.PlanetsView_startAngleX, 0.5f);
            mAngleY = typedArray.getFloat(R.styleable.PlanetsView_startAngleY, 0.5f);
            setLightColor(typedArray.getColor(R.styleable.PlanetsView_lightColor, Color.WHITE));
            setDarkColor(typedArray.getColor(R.styleable.PlanetsView_darkColor, Color.BLACK));
            setRadiusPercent(typedArray.getFloat(R.styleable.PlanetsView_radiusPercent, radiusPercent));
            setScrollSpeed(typedArray.getFloat(R.styleable.PlanetsView_scrollSpeed, 2f));
            typedArray.recycle();
        }
        ...
        this.post(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < planetAdapter.getCount(); i++) {
                    ...
                    if (i != planetAdapter.getCount() - 1){
                        View view = planetAdapter.getView(getContext(), i, PlanetsView.this);
                        planetModel.setView(view);
                        mPlanetCalculator.add(planetModel);
                        // 点击事件监听
                        addListener(view, i);
                    }else {
                        View view = planetAdapter.getSpecialView(getContext(), PlanetsView.this);
                        planetModel.setView(view);
                        mPlanetCalculator.add(planetModel);
                        // 点击事件监听
                        addListener(view, i);
        ...
        if (!view.hasOnClickListeners() && onTagClickListener != null) {
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    onTagClickListener.onItemClick(PlanetsView.this, v, position);
                }
            });
        }
    }
    ...
    public PlanetsView(Context context, AttributeSet attrs, int defStyleAttr) {
