package cai.project.module.common_view;
    ...
    public static void show(Context context, int largeIcon,
        ...
        if (cls != null) {
            ...
            if (Activity.class.isAssignableFrom(cls)){
                ...
                pendingIntent = PendingIntent.getService(context, RequestCode, intent, FLAG_UPDATE_CURRENT);
            }else if (BroadcastReceiver.class.isAssignableFrom(cls)){
                pendingIntent = PendingIntent.getBroadcast(context, RequestCode, intent, FLAG_UPDATE_CURRENT);
            }
        }
        //获取通知服务管理器
        ...
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), largeIcon == 0 ? R.drawable.iocn_snow : largeIcon)) //设置自动收报机和通知中显示的大图标。
                .setSmallIcon(smallIcon == 0 ? R.drawable.iocn_snow : smallIcon) // 小图标
...
//                .setTimeoutAfter(3000)//指定取消此通知的时间（如果尚未取消）。
    ...
    public static void show(Context context, int largeIcon,
                            ...
                            String subText, String contentText,
                            int priority, String ticker, RemoteViews view,
                            int notifyId, String channelId, Class<?> cls,Class<?> delCls) {
        //flags
        // FLAG_ONE_SHOT:表示此PendingIntent只能使用一次的标志
        // FLAG_IMMUTABLE:指示创建的PendingIntent应该是不可变的标志
        // FLAG_NO_CREATE : 指示如果描述的PendingIntent尚不存在，则只返回null而不是创建它。
        ...
        PendingIntent pendingDelIntent = null;
        //添加删除意图
        if (delCls != null){
            Intent intent = new Intent(context, delCls);
            if (Activity.class.isAssignableFrom(delCls)){
                pendingDelIntent = PendingIntent.getActivity(context, RequestCode, intent, FLAG_UPDATE_CURRENT);
            }else if (Service.class.isAssignableFrom(delCls)){
                pendingDelIntent = PendingIntent.getService(context, RequestCode, intent, FLAG_UPDATE_CURRENT);
            }else if (BroadcastReceiver.class.isAssignableFrom(delCls)){
                pendingDelIntent = PendingIntent.getBroadcast(context, RequestCode, intent, FLAG_UPDATE_CURRENT);
            }
        }
...
//                .setGroupSummary(true)//将此通知设置为一组通知的组摘要
//                .setGroup(NEW_GROUP)//使用组密钥
                .setDeleteIntent(pendingDelIntent)//当用户直接从通知面板清除通知时 发送意图
//                .setFullScreenIntent(pendingIntent,true)
//                .setContentInfo("大文本")//在通知的右侧设置大文本。
//                .setContent(RemoteViews RemoteView)//设置自定义通知栏
//                .setColor(Color.parseColor("#ff0000"))
